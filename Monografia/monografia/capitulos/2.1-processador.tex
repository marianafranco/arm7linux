...
\item Modos simples de endereçamento de memória.
...
Algumas características importantes do \emph{pipeline} do ARM7TDMI:
...
\item A execução de uma instrução de \emph{branch} através da modificação do PC provoca a descarga, eliminação, de todas as outras instruções do \emph{pipeline}.

\item Uma instrução no estágio \emph{execute} será completada mesmo se acontecer uma interrupção. As outras instruções no \emph{pipeline} serão abandonadas e o processador começará a preencher o \emph{pipeline} a partir da entrada apropriada no vetor de interrupção.
...
Os processadores ARM possuem 7 modos de operação, como apresentado na tabela \ref{table:modos}.
...
\hline
\end{tabular}
\caption{Modos de operação \cite{ArchManual}} \label{table:modos}
\end{center}
\end{table}
...
Além desses dois registradores, o modo FIQ possui mais cinco registradores especiais: R8\textunderscore fiq-R12\textunderscore fiq. Isso significa que quando o processador muda para o modo FIQ, o programa não precisa salvar os registradores de R8 à R12.
...
\item Alguma outra instrução aritmética, lógica ou \emph{move}, onde o registrador de destino não é o R15 (PC).
\end{itemize}
...
\item Quando o bit T é desativado, o processador é executado em estado ARM.
\end{itemize}
...
Os bits M[4:0] determinam o modo de operação. Nem todas as combinações dos bits de modo definem um modo válido, portando deve-se tomar cuidado para usar somente as combinações mostradas na tabela \ref{table:bit_tipo}.
...
O processador ARM7TDMI também pode ativar a \emph{flag} de interrupção para desabilitar próximas interrupções.
...
\item Desativa as \emph{flags} de interrupção que foram ativadas na entrada.
\end{enumerate} 
...
\item Desabilita as interrupções normais ativando o bit I do CPSR. Interrupções FIQ não são desabilitadas e podem continuar ocorrendo.
...
Para retornar da operação de interrupção, é usada a seguinte instrução para restaurar o PC (a partir do R14\textunderscore svc) e o CPSR (a partir do SPSR\textunderscore svc):
...
\begin{enumerate}
\item Copia o endereço da próxima instrução a ser executada + 4 no registrador LR\textunderscore irq (R14\textunderscore irq). Isso significa que o LR\textunderscore irq irá apontar para a segunda instrução a partir do ponto de pedido da interrupção.
...
\item Reforça o estado ARM colocando o bit T do CPSR a zero.
...
\subsubsection{Alocação de Registradores}

O compilador tenta alocar um registrador do processador para cada variável local que encontra em uma função C. Ele tenta usar o mesmo registrador para diferentes variáveis locais se a utilização das variáveis não se sobrepõem. Quando há mais variáveis locais que registradores disponíveis, o compilador armazena as variáveis em excesso na pilha do processador \cite{Sloss2004}.


\subsubsection{Chamadas de Função}

A \textit{ARM Procedure Call Standard} (APCS) define como passar argumentos de função e obter valores de retorno.

Os primeiros quatro argumentos inteiros são passadas nos quatro primeiros registradores ARM: R0, R1, R2 e R3. Argumentos inteiros posteriores são colocados na pilha, como na figura \ref{fig:functionregisters}. Se o valor de retorno for inteiro, este é obtido através do registrador R0 \cite{Sloss2004}.

Esta descrição abrange apenas os argumentos de tipo inteiro ou ponteiro. Argumentos que ocupam o espaço de duas palavras, como \textit{long long} e \textit{double}, são passados em um par de registradores consecutivos e retornam em R0, R1.
