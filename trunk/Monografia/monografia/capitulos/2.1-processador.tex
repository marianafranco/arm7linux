...
A arquitetura de \emph{pipeline} aumenta a velocidade do fluxo de instruções para o processador, pois permite que várias operações ocorram simultaneamente, fazendo o processador e a memória operarem continuamente. 
...
\begin{figure}[ht]
	\begin{center}
		...
		\caption{\emph{Pipeline} do ARM7TDMI\label{fig:pipeline_blockdiagram}}
	\end{center}
\end{figure}
...
\item O processador só processa a instrução quando essa passa completamente pelo estágio execução (\emph{execute}). Ou seja, somente quando a quarta instrução é buscada (\emph{fetched}).
...
O conjunto de registradores para o modo Usuário (o mesmo usado no modo Sistema) contém 16 registradores diretamente acessíveis, R0 à R15. Um registrador adicional, o CPSR (\emph{Current Program Status Register}), contem os bits de \emph{flag} e de modo. 
...
Os bits N, Z, C e V são \emph{flags} de condição, e é possível alterá-los através do resultado de operações lógicas ou aritméticas. 
...
\item V: Possui dois significados:
	\begin{itemize}
	\item Para adição ou subtração, V contem 1 caso tenha ocorrido um \emph{overflow} considerando os operandos e o resultado em complemento de dois.
...
Os oito primeiros bits de um PSR (\emph{Program Status Register}) são conhecidos como bits de controle. Eles são:
...
\hline
\end{tabular}
\caption{Vetor de interrupção \label{interrupt_vector}}
\end{center}
\end {table}


Deve-se notar olhando para a tabela a cima, que existe espaço suficiente para apenas uma instrução entre cada vetor de interrupção (4 bytes). Estes são inicializados com instruções de desvio (\emph{branch}).


\subsubsection{Prioridade das Interrupções}

Quando várias interrupções acontecem ao mesmo tempo, uma prioridade fixa do sistema determina a ordem na qual elas serão manipuladas. Essa prioridade é listada na tabela a baixo:
...
\end{tabular}
\caption{Ordem de prioridade das interrupções} \label{table:prior_interrupt}
\end{center}
\end{table}

\subsubsection{Entrada de interrupção}

Executar uma interrupção necessita que o processador preserve o estado atual: em geral, o conteúdo de todos os registradores (especialmente PC e CPSR) devem ser o mesmo depois da interrupção.

O processador ARM usa os registradores adicionais de cada modo para ajudar a salvar o estado do processador. Quando uma interrupção acontece, o R14 e o SPSR são usados para guardar o estado atual da seguinte maneira:
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.90]{figuras/interrupt.jpg}
		\caption{Esquema de uma interrupção no ARM7TDMI \label{fig:interrupt}}
	\end{center}
\end{figure}
...
Quando uma interrupção é completada deve-se:
...
Uma interrupção de software é uma interrupção inicializada inteiramente por um programa para entrar no modo Supervisor e assim poder utilizar alguma rotina particular, como operações de entrada e saída do sistema.

Quando uma interrupção de software é executada, as seguintes ações são realizadas:
...
Interrupções de hardware são mecanismos que permitem que um sinal externo (pedido de interrupção) interrompa a execução normal do programa corrente e desvie a execução para um bloco de código chamado de rotina de interrupção.
